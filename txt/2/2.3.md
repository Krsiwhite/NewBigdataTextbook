...
&emsp;&emsp;对于第一个问题，是由DFSZKFailoverController负责完成的，DFSZKFailoverController作为NameNode机器上一个独立的进程启动 ，启动的时候会创建 HealthMonitor 和 ActiveStandbyElector 这两个主要的内部组件，DFSZKFailoverController 在创建 HealthMonitor 和 ActiveStandbyElector 的同时，也会向 HealthMonitor 和 ActiveStandbyElector 注册相应的回调方法。

&emsp;&emsp;HealthMonitor 初始化完成之后会启动内部的线程来定时调用对应 NameNode 的 HAServiceProtocol RPC 接口的方法，对 NameNode 的健康状态进行检测。
HealthMonitor 如果检测到 NameNode 的健康状态发生变化，会回调 DFSZKFailoverController 注册的相应方法进行处理。
如果 DFSZKFailoverController 判断需要进行主备切换，会使用 ActiveStandbyElector 来进行自动的主备选举。

&emsp;&emsp;ActiveStandbyElector 与 QuorumPeerMain 进行交互完成自动的主备选举。
ActiveStandbyElector 在主备选举完成后，会回调 DFSZKFailoverController 的相应方法来通知当前的 NameNode 成为主 NameNode 或备 NameNode。
DFSZKFailoverController 调用对应 NameNode 的 HAServiceProtocol RPC 接口的方法将 NameNode 转换为 Active 状态或 Standby 状态。


&emsp;&emsp;对于第二个问题，是由共享存储系统实现的，共享存储系统保存了NameNode在运行过程中所产生的HDFS的元数据。Active NameNode和Standby NameNode通过共享存储系统实现元数据同步。在进行主备切换的时候，新的主NameNode在确认元数据完全同步之后才能继续对外提供服务，NameNode 共享存储方案，比如shared NAS+NFS、BookKeeper、BackupNode和QJM(Quorum Journal Manager)等等，我们这里采用的是QJM，本部分也只针对基于 QJM 的共享存储方案的内部实现原理进行分析。

&emsp;&emsp;我们在2.2.3节中提到，Edits的作用是防止突然宕机，fsimage内容未持久化保存，导致下次重新启动时读取到磁盘中未被同步的fsimage文件，最终造成一致性问题，而在HA-HDFS中，备用Standby NameNode接替Active NameNode的过程与此类似，都需要保证数据一致性，他们保证一致性的机制也类似，都是利用EditLog。

&emsp;&emsp;既然要利用EditLog，那么首先Standby NameNode和Active NameNode之间就要实现EditLog的互通，QJM是怎么做的呢？QJM共享存储采用多个称为JournalNode的节点组成的JournalNode集群来存储EditLog。每个JournalNode保存同样的EditLog副本（需要注意的是，fsimage并不会通过JournalNode共享，不会在JournalNode集群中保存，备用节点的fsimage是在初始化HA-HDFS时从主节点拷贝来的。）。每次NameNode写EditLog的时候，除了向本地磁盘写入EditLog之外，也会并行地向JournalNode集群之中的每一个JournalNode发送写请求，只要有超过一半的JournalNode节点返回成功就认为向JournalNode集群写入EditLog成功，Standby NameNode会定时的从JournalNode集群中同步EditLog。这样的写入方式保证了JournalNode集群中总有最新的EditLog。

&emsp;&emsp;确保了EditLog互通后，那么接下来就要开始利用这些EditLog了，当处于Standby状态的NameNode转换为Active状态的时候，有可能上一个 Active NameNode 发生了异常退出，那么 JournalNode 集群中各个 JournalNode 上的 EditLog 就可能会处于不一致的状态（在写入EditLog时只需要2N+1台的集群只需要超过一半的节点确认，故剩下的节点仍可能不一致），所以首先要做的事情就是让 JournalNode 集群中各个节点上的 EditLog 恢复为一致。另外如前所述，当前处于 Standby 状态的 NameNode 的内存中的文件系统中数据有很大的可能是落后于挂掉的 Active NameNode 的，所以在 JournalNode 集群中各个节点上的 EditLog 达成一致之后，接下来要做的事情就是从 JournalNode 集群上补齐落后的 EditLog。只有在这两步完成之后，当前新的 Active NameNode 才能安全地对外提供服务。

### 前面不动，后面接着写，your turn

（1）ZKFC定时检测相应主机上的namenode状态...当宕机时ZKFC会通知QuorumPeerMain选举另一台为active，QuorumPeerMain通知被选中的namenode主机上对应的ZKFC,zkfc切换namenode为active

（2）JournalNode保存edits，主namenode每写一次edits都要向jour..集群也同步写，备namenode定时同步,fsimage不存储在jour...中，备namenode的fsimage是在最开始初始化ha的时候拷贝过来的。当备切换到主时需要从jour...中同步edits
