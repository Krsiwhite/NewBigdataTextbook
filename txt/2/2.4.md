...
HBase作为Hadoop生态中的分布式数据库，构建在HDFS和ZooKeeper之上，如同一位精密的钟表师，需要协调多个组件才能完美运转。在安装时，请确保正确完成了上文ZooKeeper和 HDFS 集群的部署，HBase部署过程应该是：
...
​	配置环境变量。修改`/etc/profile`文件，在`/etc/profile`文件的内容修改为：
...
​	可看到此时`m1`上运行着一个HMaster和HRegionServer两个进程。在`m1`上，HMaster为集群管理进程，负责元数据操作、Region分配和故障转移，而HRegionServer为数据服务进程，处理客户端读写请求，管理Region数据。
